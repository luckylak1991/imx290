...
static void AFCORE_GetAFOPDWIND(PAF_CTRL_PARAM pCTRL)
{
	...
	pCTRL->stFILT.RPFa = (LONG)(((FLOAT)(MAKEDWORD(rVal[ 0], rVal[ 1])>>1) + slaveFLTa[0]/2) * afWinRatio);
	pCTRL->stFILT.LPFa = (LONG)(((FLOAT)(MAKEDWORD(rVal[ 2], rVal[ 3])>>1) + slaveFLTa[1]/2) * afWinRatio);
	pCTRL->stFILT.VPFa = (LONG)(((FLOAT)(MAKEDWORD(rVal[ 4], rVal[ 5])>>1) + slaveFLTa[2]/2) * afWinRatio);
	pCTRL->stFILT.HPFa = (LONG)(((FLOAT)(MAKEDWORD(rVal[ 6], rVal[ 7])>>1) + slaveFLTa[4]/2) * afWinRatio);
	pCTRL->stFILT.RPFb = (LONG)(((FLOAT)(MAKEDWORD(rVal[ 8], rVal[ 9])>>1) + slaveFLTb[0]/2) * afWinRatio);
	pCTRL->stFILT.LPFb = (LONG)(((FLOAT)(MAKEDWORD(rVal[10], rVal[11])>>1) + slaveFLTb[1]/2) * afWinRatio);
	pCTRL->stFILT.VPFb = (LONG)(((FLOAT)(MAKEDWORD(rVal[12], rVal[13])>>1) + slaveFLTb[2]/2) * afWinRatio);
	pCTRL->stFILT.HPFb = (LONG)(((FLOAT)(MAKEDWORD(rVal[14], rVal[15])>>1) + slaveFLTb[4]/2) * afWinRatio);

	MISP_MultiRead(MISP_LOCAL_ID, 0x6E8, (PBYTE)rVal, 16);
	pCTRL->stFILT.CNTa = (LONG)(((FLOAT)(MAKEDWORD(rVal[0], rVal[1])>>1) + slaveFLTa[5]/2) * afWinRatio);
	pCTRL->stFILT.BPFa = (LONG)(((FLOAT)(MAKEDWORD(rVal[2], rVal[3])>>1) + slaveFLTa[3]/2) * afWinRatio);
	pCTRL->stFILT.CNTb = (LONG)(((FLOAT)(MAKEDWORD(rVal[4], rVal[5])>>1) + slaveFLTb[5]/2) * afWinRatio);
	pCTRL->stFILT.BPFb = (LONG)(((FLOAT)(MAKEDWORD(rVal[6], rVal[7])>>1) + slaveFLTb[3]/2) * afWinRatio);
#else
	MISP_MultiRead(MISP_LOCAL_ID, 0x6f0, (PBYTE)rVal, 32);
