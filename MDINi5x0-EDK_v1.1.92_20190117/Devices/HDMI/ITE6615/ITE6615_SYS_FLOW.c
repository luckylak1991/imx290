...
#define MAX_HDCP_Need_Restart_Count 15
...
void iTE6615_vid_chg(iTE6615_STATEV_Type NewState)
{
    ...
    switch(iTE6615_DATA.STATEV)
    {
        case iTE6615_STATEV_Reset:
            ...
            iTE6615_DATA.Flag_RX_CED_ERROR_OCCUR = 0;
            iTE6615_aud_chg(iTE6615_STATEA_Reset);
...
void iTE6615_INT_SCDC_UpdateFlagChg_Handler( u8 rddata )
{
    ...
    if (rddata & 0x02) {
        ...
        if (iTE6615_SCDC_Read(0x50, 7)) {
            ...
            {
                HDMITX_SCDC_PRINTF(("Flag_RX_CED_ERROR_OCCUR = 1 ---------------------\r\n"));
                iTE6615_DATA.Flag_RX_CED_ERROR_OCCUR = 1;
            }
			else
			{
                HDMITX_SCDC_PRINTF(("Flag_RX_CED_ERROR_OCCUR = 0 ---------------------\r\n"));
                iTE6615_DATA.Flag_RX_CED_ERROR_OCCUR = 0;
            }
...
void iTE6615_Set_AVI_Infoframe(void)
{
    ...
    if(iTE6615_DATA.config.Output_ColorSpace == RGB444)
    {
        ...
        if(iTE6615_DATA.config.Output_RGB_QuantizationRange == IT6615_RGB_RANGE_FULL)
        {
    ...
    else
    {
        ...
        hdmitxset(0x59, 0xC0, iTE6615_DATA.config.Output_YCC_Colorimetry); // Colorimetry = ITU709
    }
    ...
    if( iTE6615_DATA.config.Output_ExtendedColorimetry_Enable  &&
        ( ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==xvYCC601    &&  (iTE6615_DATA.sink.ExtColCap&0x01)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==xvYCC709    &&  (iTE6615_DATA.sink.ExtColCap&0x02)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==sYCC601     &&  (iTE6615_DATA.sink.ExtColCap&0x04)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==AdobeYCC601 &&  (iTE6615_DATA.sink.ExtColCap&0x08)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==AdobeRGB    &&  (iTE6615_DATA.sink.ExtColCap&0x10)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==BT2020cYCC  &&  (iTE6615_DATA.sink.ExtColCap&0x20)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==BT2020YCC   &&
          (((iTE6615_DATA.sink.ExtColCap&0x40)==0x00 && iTE6615_DATA.config.Output_ColorSpace!=RGB444) || ((iTE6615_DATA.sink.ExtColCap&0x80)==0x00 && iTE6615_DATA.config.Output_ColorSpace==RGB444))) ))
    {
...
void iTE6615_Set_VendorSpecific_Infoframe()
{
    ...
    if( iTE6615_DATA.config.Output_ExtendedColorimetry_Enable == 1 &&
      ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==xvYCC601 ||
        iTE6615_DATA.config.Output_ExtendedColorimetry_Format==xvYCC709 ) ) {
        chgtxbank(1);
        hdmitxwr(0xB0, 0x00);
        hdmitxwr(0xB1, 0xB0);
...
void iTE6615_Set_HDMI2_SCDC( void )
{
    ...
    u8 Enable_HDMI2_Encode=0, Enable_HDMI2_Scramble, Enable_HDMI2_ClkRatio401;
...
void iTE6615_TASK_HDCP_RetryAuth( void )
{
	...
	// longer for CTS, CTS need to re-auth in 2 second , but if Intervals of re-auth
	// about 300ms will may occur CTS tool fail, so add longer
...
{
	ITE6615_Tick = TRUE;			// tick it6615 loop
}
//--------------------------------------------------------------------------------------------------
void	ITE6615_SetFrmt(void)
{
	// set AVI InfoFrame
	BYTE VIC;
//	BYTE Y;
	...
	switch(GetOUT_MainFrmt()) {
		...
		case MISP_VIDOUT_1280x720p30:   VIC=  4;  break;  // unknown, substitute with 1280x720p60
		case MISP_VIDOUT_1280x720p25:   VIC= 19;  break;  // unknown, substitute with 1280x720p50
		case MISP_VIDOUT_1280x720p24:   VIC= 19;  break;  // unknown, substitute with 1280x720p50
		...
		case MISP_VIDOUT_1920x2160p60:  VIC= 97;  break;
		case MISP_VIDOUT_1920x2160p50:  VIC= 96;  break;
#endif
		default:                   VIC=  5;  break;  // unknown, substitute with 1920x1080p60
	}
...
/*	switch(stVideo[0].stOUT_m.mode) {
	...
	ITE6615_SetAviInfoVIC(VIC);
//	ITE6615_SetAviInfoColor(Y);

	

#if defined(__MDIN_i540_4K60_V10__)
	if((GetOUT_MainFrmt() == MISP_VIDOUT_1920x2160p60) || (GetOUT_MainFrmt() == MISP_VIDOUT_1920x2160p50))
	{
		iTE6615_Customer_Set_InputDualMode( 1, 1);
//		iTE6615_Customer_Set_InputDualMode((stVideo[0].mst_out_en)?  1:  0,  (stVideo[0].mst_out_en)?  1:  0);
		iTE6615_sys_chg(iTE6615_STATES_Unplug);
	}
#endif
...
{
	iTE6615_DATA.config.Output_ColorSpace = outCSC;
	iTE6615_sys_chg(iTE6615_STATES_Unplug);
}
...
{
	return iTE6615_DATA.config.Output_ColorSpace;
}
