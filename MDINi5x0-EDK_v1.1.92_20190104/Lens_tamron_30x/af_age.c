...
static LONG AALENS_GetYSUM(void)
{
	...
	WND[0] = MAKEDWORD((rVal[0]&0xff), rVal[1])>>2;
	WND[1] = MAKEDWORD((rVal[2]&0xff), rVal[3])>>2;
	WND[2] = MAKEDWORD((rVal[4]&0xff), rVal[5])>>2;
	WND[3] = MAKEDWORD((rVal[6]&0xff), rVal[7])>>2;
	WND[4] = MAKEDWORD((rVal[8]&0xff), rVal[9])>>2;

	return (WND[0]+WND[1]+WND[2]+WND[3]+WND[4])/5;
}
