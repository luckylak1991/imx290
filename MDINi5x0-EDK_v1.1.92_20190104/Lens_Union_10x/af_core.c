...
#if 0 // for testing
	frameCnt = (frameCnt+1)%30;
	DWORD histo[8];
	if(frameCnt ==0) {
		Histogram_Read(0, 8, &histo[0]);
		UARTprintf("hist : %d : %d : %d : %d : %d : %d : %d : %d\r\n", histo[0], histo[1], histo[2], histo[3], histo[4], histo[5], histo[6], histo[7]);
	}
#else
#if 1
#if 0
...
#else
	DWORD histo[4];
	Histogram_Read(0, 4, &histo[0]);
	//UARTprintf("hist : %d : %d : %d : %d\r\n", histo[0], histo[1], histo[2], histo[3]);
	if((histo[0] > histo[1]) && (histo[3] > histo[2])) {
		if (pCTRL->stFILT.CNTa<stAFTUNE.rSPOTGain) {
			pCTRL->nSPT = 0;
		} else {
			if(pCTRL->nSPT < stAFTUNE.rSPOTCnt)
...
static LONG AFCORE_CtrlMeanBUFF(PLONG pBUFF, BOOL nID)
{
	...
	return MAX((LONG)((FLOAT)sum/(FLOAT)NumOfBUFF+0.5),30); // mimitest
}
...
#if __MISP100_AF_DBG_CORE__ == 1
	if ((ODM_GetDEBUGMode()==0x03) || (ODM_GetDEBUGMode()==0x04)) {
		UARTprintf("%s:axb:%6d anB:%6d\r\n", __FUNCTION__,stAFAUTO.xFLTb, stAFAUTO.nFLTb);
	}
#endif
}
...
static void AFCORE_CtrlTriggerByLUMA(PAF_CTRL_PARAM pCTRL) //check YSUM ratio -- If ysum ratio is too big... reset the counter ( AE Change or block)
{
	...
	if (stAFAUTO.nGAIN==AECTRL_GetGAINPMax())
...
static void AFCORE_CtrlTriggerByDATA(PAF_CTRL_PARAM pCTRL)
{
	...
	rFLTh = (LONG)((FLOAT)pCTRL->stFILT.GAIN/(FLOAT)(AECTRL_GetGAINPMax()/8)+20.5); //16384
	rFLTb = (LONG)((FLOAT)pCTRL->stFILT.GAIN/(FLOAT)(AECTRL_GetGAINPMax()/8)+20.5);
...
#if __MISP100_AF_DBG_CORE__ == 1
	if ((stAFAUTO.RATEh>=rFLTh||stAFAUTO.RATEb>=rFLTb)&&((ODM_GetDEBUGMode()==0x03) || (ODM_GetDEBUGMode()==0x04))) {
		UARTprintf("%s:nH:%06d rH:%06d ", __FUNCTION__,stAFAUTO.RATEh, rFLTh);
...
static void AFCORE_CtrlTriggerByNORM(PAF_CTRL_PARAM pCTRL)
{
	...
	rate = ((FLOAT)(AECTRL_GetGAINPMax()/8)/(FLOAT)pCTRL->stFILT.GAIN)*stAFTUNE.rFLTh+0.5; //16384
	rFLTh = MIN(MAX((SHORT)rate,stAFTUNE.rFLTh/2),stAFTUNE.rFLTh);

	rate = ((FLOAT)(AECTRL_GetGAINPMax()/8)/(FLOAT)pCTRL->stFILT.GAIN)*stAFTUNE.rFLTb+0.5;
...
#if __MISP100_AF_DBG_CORE__ == 1
	if ((stAFAUTO.RATEh>=rFLTh||stAFAUTO.RATEb>=rFLTb)&&((ODM_GetDEBUGMode()==0x03) || (ODM_GetDEBUGMode()==0x04))) {
		UARTprintf("%s:axB:%6d anB:%6d ", __FUNCTION__, stAFAUTO.xFLTb, stAFAUTO.nFLTb);
		UARTprintf("nH:%06d rH:%06d ", stAFAUTO.RATEh, rFLTh);
		UARTprintf("nB:%06d rB:%06d\r\n", stAFAUTO.RATEb, rFLTb);
	}
#endif
}
...
static void AFCORE_CtrlTriggerByYSUM(PAF_CTRL_PARAM pCTRL)
{
	...
	if (stAFAUTO.nGAIN==AECTRL_GetGAINPMax()) rGAPy = 20;
	...
	nGAIN = AECTRL_GetGAINPMax();	stAFAUTO.CNTy = 0;
...
#if __MISP100_AF_DBG_CORE__ == 1
	if ((ODM_GetDEBUGMode()==0x03) || (ODM_GetDEBUGMode()==0x04)) {
		UARTprintf("%s:nY:%05d xY:%05d dY:%05d nG:%05d\r\n", __FUNCTION__,
...
#if __MISP100_AF_DBG_CORE__ == 1
	if ((ODM_GetDEBUGMode()==0x03) || (ODM_GetDEBUGMode()==0x04) || (ODM_GetDEBUGMode()==0x05)) {
...
#if __MISP100_AF_DBG_CORE__ == 1
	if ((ODM_GetDEBUGMode()==0x03) || (ODM_GetDEBUGMode()==0x04)) UARTprintf("%s\r\n",__FUNCTION__);
#endif
...
#if __MISP100_AF_DBG_CORE__ == 1
	if ((ODM_GetDEBUGMode()==0x03) || (ODM_GetDEBUGMode()==0x04))
		UARTprintf("%s:dF:%06d yG:%06d\r\n",__FUNCTION__, diff, yGAP);
#endif
